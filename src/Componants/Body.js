...
const Body = () => {
  ...
  const fetchData = async () => {
   ...
   console.log(json?.data?.cards[2].card?.card?.gridElements?.infoWithStyle?.restaurants)
    ...
    setFilterdResturant(json?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants);
    console.log(json?.data?.cards[2]?.card?.card?.gridElements?.infoWithStyle?.restaurants);
  ...
  if(onlineStatus === false) return <h1 className="offline text-center my-[50px] font-mono text-xl">Look's like Your Are Offline!!! Check your internet 😟</h1>
  ...
  return listOFResturant.length === 0 ? (
    ...
    <div className="main-container w-full px-8 py-4">
      ...
      <div className="filter lg:flex md:flex py-2 w-100% justify-center max-w-[1080px] hidden">
        ...
        <label className="text-orange-900 leading-8 text-[20px] mx-2" htmlFor="">User :</label>
        <input type="text" value={loggedInUser} onChange={(e) => setUserName(e.target.value)} className="bg-orange-200 px-2 outline-orange-900 rounded-md" />
      </div>
      <div className="res-container w-full flex flex-wrap px-4 py-4 justify-center gap-8">
        {filterdResturant.map((restaurants) => (
          ...
          > {restaurants.info.promoted ? (<ResturantCardPromoted resData={restaurants}/>) : <ResturantCard resData={restaurants?.info} />}
